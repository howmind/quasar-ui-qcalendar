...
import { createDayList, getDayIdentifier, createNativeLocaleFormatter } from '../utils/Timestamp.js'
...
export default {
    ...
    isMiniMode() {
      return (
        ...
        (this.miniMode === 'auto' &&
          ...
          this.$q.screen.lt[this.breakpoint])
      )
    }
  },
  ...
  methods: {
    isOutside(day) {
      ...
      return (
        dayIdentifier < getDayIdentifier(this.parsedStart) ||
    ...
    __renderContainer(h) {
      ...
      if (this.animated === true) {
        const transition =
          'q-transition--' + (this.direction === 'prev' ? this.transitionPrev : this.transitionNext)
        return h(
          'transition',
          {
            props: {
      ...
      return h(
        ...
        [
          h(
            ...
            [this.showWorkWeeks === true && this.__renderWorkWeekHead(h), this.__renderHeadDays(h)]
          )
        ]
      )
    },
      ...
      return h(
        'div',
        {
          ...
          staticClass:
            ...
            (disabled === true ? ' q-disabled-day disabled' : ''),
          style: {
            minWidth: width,
            maxWidth: width
          },
    ...
    __renderHeadDayLabel(h, day, shortWeekdayLabel) {
      ...
      return h(
        ...
        this.isMiniMode === true && this.shortWeekdayLabel === true
          ? weekdayLabel.charAt(0)
          : weekdayLabel
      )
    },
    ...
    __renderDay(h, day) {
      ...
      const hasMonth =
        outside === false &&
        this.days.find((d) => d.month === day.month).day === day.day &&
      ...
      return h(
        ...
        [
          //
          // 2022.08 enhance
          // set day slot as background template
          // so draw it firstly, then draw others elements
          //
          h(
            'div',
            {
              staticClass:
                'q-calendar-weekly__day--content full-width' +
                (this.isMiniMode === true ? ' row justify-center items-center' : '')
            },
            slot ? slot(scope) : ''
          ),
          this.__renderDayLabel(h, day),
          this.isMiniMode !== true && this.showDayOfYearLabel && !hasMonth
            ? this.__renderDayOfYearLabel(h, day)
            : '',
    ...
    __renderDayLabel(h, day) {
      ...
      const selectedDate =
        // this.isMiniMode &&
        this.selectedDates && this.selectedDates.length > 0 && this.selectedDates.includes(day.date)
        ...
        : h(
            QBtn,
            {
              ...
              props: {
                ...
                disable:
                  day.disabled === true || (this.enableOutsideDays !== true && outside === true)
              },
              style: {
  ...
  render(h) {
    return h(
      ...
      [!this.hideHeader && this.__renderHead(h), this.__renderContainer(h)]
    )
  }
}
